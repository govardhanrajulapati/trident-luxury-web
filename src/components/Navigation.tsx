  ...
  return (
    <nav className={`fixed top-0 w-full z-50 transition-all duration-300 ${
      isScrolled 
        ...
        : 'bg-navy-950/10 backdrop-blur-sm'
    }`}>
          ...
          <Link to="/" className="flex items-center space-x-3 z-10">
            ...
            <div className="flex flex-col">
              <span className={`font-playfair font-bold text-xl transition-colors ${
                isScrolled ? 'text-navy-900' : 'text-white drop-shadow-lg'
              }`}>
                Trinethra
              </span>
              <span className={`text-sm font-medium transition-colors ${
                isScrolled ? 'text-navy-600' : 'text-white/90 drop-shadow-md'
              }`}>
                Developers
              </span>
            </div>
              ...
              <Link
                ...
                className={`font-medium transition-all duration-200 relative group ${
                  location.pathname === item.href
                    ? (isScrolled ? 'text-gold-600' : 'text-gold-400 drop-shadow-lg')
                    : (isScrolled ? 'text-navy-700 hover:text-gold-600' : 'text-white hover:text-gold-400 drop-shadow-md hover:drop-shadow-lg')
                }`}
              >
                {item.name}
            ...
            <div className="flex items-center space-x-4 text-sm">
              <a
                ...
                className={`flex items-center space-x-1 transition-colors ${
                  isScrolled ? 'text-navy-600 hover:text-gold-600' : 'text-white/90 hover:text-gold-400 drop-shadow-md'
                }`}
              >
            ...
            <Sheet open={isOpen} onOpenChange={setIsOpen}>
              <SheetTrigger asChild>
                <Button variant="ghost" size="icon" className={`${
                  isScrolled ? 'text-navy-900 hover:bg-navy-100' : 'text-white hover:bg-white/20 drop-shadow-lg'
                } transition-colors`}>
